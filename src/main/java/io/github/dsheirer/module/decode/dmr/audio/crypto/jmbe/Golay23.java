package io.github.dsheirer.module.decode.dmr.audio.crypto.jmbe;

/*
 * Vendored, unmodified except for the package declaration, from
 * github.com/DSheirer/jmbe (GPLv3, same license and author as this project).
 *
 * Why this is here: jmbe's codec implementation (where this class lives) is
 * not a compile-time dependency of SDRTrunk - it's built by the end user and
 * loaded as a runtime plugin, exposing only the black-box IAudioCodec
 * interface (raw frame bytes in, PCM audio out). DMR/P25 decryption needs
 * access to the FEC-decoded-but-still-encrypted voice parameters, a layer
 * JMBE never exposes through that interface. This class (plus Golay23,
 * Golay24) is vendored here so DmrAmbeFrameCodec can replicate that decode
 * step, decrypt it, and re-encode a synthetic frame for the existing,
 * untouched IAudioCodec.getAudio() call to consume normally.
 */
